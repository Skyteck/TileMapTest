    ...
    public class Sprite
    {
        ...
        public virtual void Draw(SpriteBatch spriteBatch, Rectangle sr)
        {
            if (_Draw)
            {
                ...
                if (!_FlipX && !_FlipY)
                {
                    spriteBatch.Draw(_Texture, _Position, sr, _MyColor * _Opacity, _Rotation, _Center, _Scale, SpriteEffects.None, 0f);
                }
                else if (_FlipX)
                {
                    spriteBatch.Draw(_Texture, _Position, sr, _MyColor * _Opacity, _Rotation, _Center, _Scale, SpriteEffects.FlipHorizontally, 0f);
                }
                else if (_FlipY)
                {
                    spriteBatch.Draw(_Texture, _Position, sr, _MyColor * _Opacity, _Rotation, _Center, _Scale, SpriteEffects.FlipVertically, 0f);
                }
                else if (_FlipX && _FlipY)
                {
                    spriteBatch.Draw(_Texture, _Position, sr, _MyColor * _Opacity, (_Rotation + (float)Math.PI), _Center, _Scale, SpriteEffects.None, 0f);
                }
